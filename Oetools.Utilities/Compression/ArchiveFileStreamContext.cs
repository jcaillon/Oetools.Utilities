...
namespace Oetools.Utilities.Compression {
    /// <summary>
    ...
    public class ArchiveFileStreamContext
        ...
        private string TranslateFilePath(string path) {
            string filePath;
            if (Files != null) filePath = Files[path];
            else filePath = path;

            if (filePath != null) if (Directory != null) filePath = Path.Combine(Directory, filePath);
        ...
        public ArchiveFileStreamContext(
            ...
            : this(new[] {archiveFile}, directory, files) {
            if (archiveFile == null) throw new ArgumentNullException("archiveFile");
        }
        ...
        public ArchiveFileStreamContext(
            ...
            string directory,
            IDictionary<string, string> files) {
            if (archiveFiles == null || archiveFiles.Count == 0) throw new ArgumentNullException("archiveFiles");
        ...
        public virtual string GetArchiveName(int archiveNumber) {
            if (archiveNumber < ArchiveFiles.Count) return Path.GetFileName(ArchiveFiles[archiveNumber]);
        ...
        ///     <para>
        ///         If the <see cref="EnableOffsetOpen" /> flag is set, this method
        ...
        public virtual Stream OpenArchiveWriteStream(
            ...
            if (archiveNumber >= ArchiveFiles.Count) return null;

            if (string.IsNullOrEmpty(archiveName)) throw new ArgumentNullException("archiveName");
            ...
            if (EnableOffsetOpen) {
                var offset = compressionEngine.FindArchiveOffset(
                    new DuplicateStream(stream));

                // If this is not an archive file, append the archive to it.
                if (offset < 0) offset = stream.Length;

                if (offset > 0) stream = new OffsetStream(stream, offset);
        ...
        ///     returned by <see cref="GetArchiveName" />.
        /// </param>
        /// <param name="stream">
        ///     A stream that was previously returned by
        ///     <see cref="OpenArchiveWriteStream" /> and is now ready to be closed.
        /// </param>
        public virtual void CloseArchiveWriteStream(
            ...
            if (stream != null) {
                ...
                if (fileStream != null) {
                    var streamFile = fileStream.Name;
        ...
        ///     <list>
        ///         <item>
        ///             If the <see cref="Directory" /> and the <see cref="Files" /> mapping
        ///             are both null, the path is treated as relative to the current directory,
        ///             and that file is opened.
        ///         </item>
        ///         <item>
        ///             If the <see cref="Directory" /> is not null but the <see cref="Files" />
        ///             mapping is null, the path is treated as relative to that directory, and
        ///             that file is opened.
        ///         </item>
        ///         <item>
        ///             If the <see cref="Directory" /> is null but the <see cref="Files" />
        ...
        ///         </item>
        ///         <item>
        ///             If both the <see cref="Directory" /> and the <see cref="Files" />
        ...
        public virtual Stream OpenFileReadStream(
            ...
            var filePath = TranslateFilePath(path);
        ...
        /// <param name="stream">
        ///     A stream that was previously returned by
        ///     <see cref="OpenFileReadStream" /> and is now ready to be closed.
        /// </param>
        ...
        ///     <para>
        ///         If the <see cref="EnableOffsetOpen" /> flag is set, this method will
        ...
        public virtual Stream OpenArchiveReadStream(
            ...
            var archiveFile = ArchiveFiles[archiveNumber];
            Stream stream = File.Open(
        ...
        ///     <see cref="OpenArchiveReadStream" /> and is now ready to be closed.
        /// </param>
        ...
        ///     </list>
        ///     <para>
        ///         If the <see cref="ExtractOnlyNewerFiles" /> flag is set, the file
        ...
        public virtual Stream OpenFileWriteStream(
            ...
            if (file.Exists) {
                if (ExtractOnlyNewerFiles && lastWriteTime != DateTime.MinValue) if (file.LastWriteTime >= lastWriteTime) return null;

                // Clear attributes that will prevent overwriting the file.
                // (The final attributes will be set after the file is unpacked.)
                var attributesToClear =
                    FileAttributes.ReadOnly | FileAttributes.Hidden | FileAttributes.System;
                if ((file.Attributes & attributesToClear) != 0) file.Attributes &= ~attributesToClear;
            }
        ...
        /// <param name="stream">
        ///     The stream that was previously returned by
        ///     <see cref="OpenFileWriteStream" /> and is now ready to be closed.
        /// </param>
