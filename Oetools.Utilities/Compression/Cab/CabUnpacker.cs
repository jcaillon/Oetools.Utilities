...
namespace Oetools.Utilities.Compression.Cab {
    internal class CabUnpacker : CabWorker {
        ...
        internal override int CabOpenStreamEx(string path, int openFlags, int shareMode, out int err, IntPtr pv) {
            if (CabNumbers.ContainsKey(path)) {
                ...
                if (stream == null) {
                    ...
                    if (stream == null) throw new FileNotFoundException(string.Format(CultureInfo.InvariantCulture, "Cabinet {0} not provided.", cabNumber));
        ...
        private static string GetFileName(NativeMethods.FDI.NOTIFICATION notification) {
            ...
            while (Marshal.ReadByte(notification.psz1, nameBytesCount) != 0) nameBytesCount++;
            ...
            if (Path.IsPathRooted(name)) name = name.Replace("" + Path.VolumeSeparatorChar, "");
        ...
        private bool IsCabinet(Stream cabStream, out short id, out int cabFolderCount, out int fileCount) {
            ...
            try {
                ...
                var isCabinet = 0 != NativeMethods.FDI.IsCabinet(fdiHandle, streamHandle, out fdici);

                if (Erf.Error)
                    if ((NativeMethods.FDI.ERROR) Erf.Oper == NativeMethods.FDI.ERROR.UNKNOWN_CABINET_VERSION) isCabinet = false;
                    else
                        throw new CabException(
                            Erf.Oper,
                            Erf.Type,
        ...
        private int CabExtractCopyFile(NativeMethods.FDI.NOTIFICATION notification) {
            ...
            if (filter == null || filter(name)) {
                ...
                var stream = context.OpenFileWriteStream(name, notification.cb, lastWriteTime);
                if (stream != null) {
                    FileStream = stream;
