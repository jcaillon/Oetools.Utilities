...
// This file (CrcStream.cs) is part of csdeployer.
// 
    ...
    public class CrcStream : Stream {
        ...
        private static uint[] MakeCrcTable() {
            ...
            for (uint n = 0; n < 256; n++) {
                var c = Reflect(n, 8);
                c = c << 24;
                for (uint k = 0; k < 8; k++) c = (c << 1) ^ ((c & 0x80000000u) != 0 ? poly : 0);
        ...
        private static uint Reflect(uint value, int bits) {
            for (var i = 0; i < bits / 2; i++) {
                var leftBit = 1u << (bits - 1 - i);
                var rightBit = 1u << i;
                if ((value & leftBit) != 0 != ((value & rightBit) != 0)) value ^= leftBit | rightBit;
            }
            return value;
        }
    }
