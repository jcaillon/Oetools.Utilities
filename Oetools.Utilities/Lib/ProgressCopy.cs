...
// This file (ProgressCopy.cs) is part of Oetools.Utilities.
// 
    ...
    public class ProgressCopy {
        ...
        public static ProgressCopy CopyDirectory(string source, string target, EventHandler<EndEventArgs> completed, EventHandler<ProgressEventArgs> progressChanged) {
            ...
            try {
                ...
                while (folderStack.Count > 0) {
                    ...
                    foreach (var file in Directory.GetFiles(folders.Source, "*.*")) fileStack.Push(new FileToCopy(file, Path.Combine(folders.Target, Path.GetFileName(file))));
                    foreach (var folder in Directory.GetDirectories(folders.Source)) folderStack.Push(new FolderToCopy(folder, Path.Combine(folders.Target, Path.GetFileName(folder))));
                }
            } catch (Exception e) {
                if (completed != null)
        ...
        private void WcOnDownloadFileCompleted(object sender, AsyncCompletedEventArgs args) {
            if (args.Cancelled || args.Error != null) {
                if (Completed != null) Completed(this, new EndEventArgs(args.Cancelled ? CopyCompletedType.Aborted : CopyCompletedType.Exception, args.Error));
            } else {
                Next();
            }
        }
