...
// This file (FastReplacer.cs) is part of csdeployer.
// 
    ...
    public class FastReplacer {
        ...
        private void ExtractTokens(FastReplacerSnippet snippet) {
            ...
            while (last < snippet.Text.Length) {
                ...
                var start = snippet.Text.IndexOf(_tokenOpen, last, StringComparison.InvariantCultureIgnoreCase);
                if (start == -1)
                    return;
                var end = snippet.Text.IndexOf(_tokenClose, start + _tokenOpen.Length, StringComparison.InvariantCultureIgnoreCase);
                if (end == -1)
        ...
        private void ValidateToken(string token, string context, bool alreadyValidatedStartAndEnd) {
            if (!alreadyValidatedStartAndEnd) {
                ...
                var closePosition = token.IndexOf(_tokenClose, StringComparison.InvariantCultureIgnoreCase);
                if (closePosition == -1)
